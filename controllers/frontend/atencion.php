...
class Atencion extends CI_Controller
{
	...
	{		
        $data['web_css'] = array( 
		  	array('href'=>'web/frontend/css/atencion.css'),
		...
		$data['web_js'] = array(					
			...
			array('src'=>'web/frontend/js/atencion.js')
		);
        ...
        $data['tpl'] = 'frontend/atencion/index';		
		...
		if($this->input->post('nospam') == ""):
			...
			if($this->form_validation->run()==FALSE): 
				...
				$datos = array(
								...
								'ciudad'       	 => $ciudad,
								'mensaje'        => $mensaje);
				$this->egeneral_model->agregar('buzon_atencion',$datos);
				/* ENVIAR MENSAJE */
				
				$config = $this->egeneral_model->configuracion();
				$mailp = $config->email_atencion; // Principal
				$mailc = $config->email_copia; // Copia
      			$para = explode(", ",$mailp);
      			$copia = explode(", ",$mailc);
      			$htmlMensaje 	 = 'Se ha registrado un mensaje de contacto mediante el formulario de "Atención al Cliente - Elite":<br>';
     			$htmlMensaje 	.= '<b>Nombres</b>: '.$nombre;
     			$htmlMensaje 	.= '<br><b>Email</b>: '.$email;
     			$htmlMensaje 	.= '<br><b>Teléfono</b>: '.$telefono;
     			$htmlMensaje 	.= '<br><b>Ciudad</b>: '.$ciudad;
     			$htmlMensaje 	.= '<br><b>Mensaje</b>: '.$mensaje;
      			$this->solo_mail->enviarmail("Mensaje de Atención al Cliente", $para, $copia, $htmlMensaje);
