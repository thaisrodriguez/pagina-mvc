...
class backend extends CI_Controller {
	...
	function index()
	{
		...
		elseif (!$this->ion_auth->is_admin()) //remove this elseif if you want to enable this for non-admins
		{
			...
			return show_error('Necesitas ser administrador para ver esta pagina.');
		}
		else
		{
			...
			if($this->input->post('valido'))
                        {
                            ...
                            'imagen_testimonio'=>$this->input->post('vistaimagen2'),
                            'imagen_contacto'=>$this->input->post('vistaimagen'),
                            'descripcion_contacto'=>$this->input->post('descripcion_contacto')
                            );
                             $result = $this->egeneral_model->editar('configuracion', $data, '1');
                             $this->session->set_flashdata('aceptado', 'Cambios realizados correctamente.');
                             redirect('/backend/Backend/index', 'refresh');
                             exit;
                        }
			$this->data['web_js']  = array(
			...
			//$this->data['fondo'] = 'texture';
			$this->data['tpl'] = 'backend/index/index';
			...
			if($this->input->post('valido'))
                        {
                            ...
                            'trofeos'=>$this->input->post('trofeos'),
                            'linkedin'=>$this->input->post('linkedin'),
                            'puestos'=>$this->input->post('puestos'),
                            'puestos_2'=>$this->input->post('puestos_2'),
                            'conociste'=>$this->input->post('conociste'),
                            'email'=>$this->input->post('email'),
                            'email_postulantes'=>$this->input->post('e_distribuidores'),
                            'email_copia'=>$this->input->post('email_copia')
                            );
                             $result = $this->egeneral_model->editar('configuracion', $data, '1');
                             $this->session->set_flashdata('aceptado', 'Cambios realizados correctamente.');
                             redirect('/backend/Backend/general', 'refresh');
                             exit;
                        }

			$this->data['tpl'] = 'backend/index/general';
			$this->load->view('backend/layout/layout', $this->data);
	}
	...
	function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			else
			{
				...
				redirect('backend/login', 'refresh'); //use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		}
		else
		{
			...
			$this->data['fondo'] = 'texture';
			$this->data['cabeza'] = '1';
			$this->data['tpl'] = 'backend/login/login';
			$this->load->view('backend/layout/layout', $this->data);
		}
	}
	//log the user out
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			//render
			$this->data['fondo'] = 'texture';
			$this->data['tpl'] = 'backend/login/cambiarpass';
			$this->load->view('backend/layout/layout', $this->data);
		}
		else
		{
			...
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect('backend/MiCuenta', 'refresh');
			}
		}
	}
	//forgot password
	function forgot_password()
	{
		...
		else
		{
			...
			if ($forgotten)
			{
				...
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				redirect("backend/login", 'refresh'); //we should display a confirmation page here instead of the login page
			}
			else
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
		...
		if ($user)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						$this->session->set_flashdata('message', $this->ion_auth->errors());
						redirect('backend/reset_password/' . $code, 'refresh');
					}
				}
			}
		}
	...
	function edit_user()
	{
		...
		$this->data['fondo'] = 'texture';
		$this->data['tpl'] = 'backend/login/editar_usuario';
		$this->load->view('backend/layout/layout', $this->data);
	}
	// create a new group
