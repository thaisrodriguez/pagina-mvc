  ...
  {    
    ...
    if (!$this->ion_auth->logged_in()):
        ...
        if($this->input->post('valido'))
                        {
                          $orden    = $this->egeneral_model->obtenerOrden('tv');
                          ...
                          $nuevaorden = $orden->orden+1; 
                            $data = array(
                             ...
                             $result= $this->egeneral_model->agregar('tv', $data);
                             $this->session->set_flashdata('aceptado', 'Cambios realizados correctamente.');
                             redirect('/backend/Tv/index', 'refresh');
                             exit;
                        }
        $data['web_js']  = array(
        ...
        $data['tpl'] = 'backend/tv/agregar';   
        $data['ultimoid'] = $this->egeneral_model->ultimoId('tv');
        $this->load->view('backend/layout/layout', $data);
    }
    ...
    {    
      ...
      if (!$this->ion_auth->logged_in()):
          ...
          if($this->input->post('valido'))
                          {
                            ...
                            $seoo = url_title($this->input->post('titulo'),'-',true);
                              ...
                              'nombreseo' => $seoo
                               ...
                               $result= $this->egeneral_model->editar('tv', $data, $id);
                               $this->session->set_flashdata('aceptado', 'Cambios realizados correctamente.');                               
                               redirect('/backend/Tv/index', 'refresh');
                               exit;
                          }
          ...
          $data['datos'] = $this->egeneral_model->mostrar('tv',$data_id);
          $data['tpl'] = 'backend/tv/editar';       
          $this->load->view('backend/layout/layout', $data);
      }
