  ...
  {    
    ...
    if (!$this->ion_auth->logged_in()):
      ...
      $data['web_js']   = array(
        ...
        $data['tabla'] = 'oficinas';
        $data['tpl'] = 'backend/oficinas/index';       
    ...
    if (!$this->ion_auth->logged_in()):
        ...
        if($this->input->post('valido'))
                        {
                          $orden    = $this->egeneral_model->obtenerOrden('oficinas');
                          $seoo = url_title($this->input->post('pais'),'-',true);
                          $nuevaorden = $orden->orden+1; 
                            $data = array(
                             ...
                             $result= $this->egeneral_model->agregar('oficinas', $data);
                             $this->session->set_flashdata('aceptado', 'Cambios realizados correctamente.');
                             $this->egeneral_model->timeline('fa-map-marker','Oficina agregada: '.$this->input->post('pais'),time(), $this->ion_auth->get_user_id(),1);
                             redirect('/backend/Oficinas/index', 'refresh');
                             exit;
                        }
        $data['web_js']  = array(
        ...
        $data['tpl'] = 'backend/oficinas/agregar';   
        $data['ultimoid'] = $this->egeneral_model->ultimoId('clientes');
        $this->load->view('backend/layout/layout', $data);
    }
    ...
    {    
      ...
      if (!$this->ion_auth->logged_in()):
          ...
          if($this->input->post('valido'))
                          {
                            ...
                            $seoo = url_title($this->input->post('pais'),'-',true);
                              ...
                              'seo' => $seoo
                               ...
                               $result= $this->egeneral_model->editar('oficinas', $data, $id);
                               $this->session->set_flashdata('aceptado', 'Cambios realizados correctamente.');
                               $this->egeneral_model->timeline('fa-map-marker','Oficina editada: '.$this->input->post('pais'),time(), $this->ion_auth->get_user_id(),1);
                               redirect('/backend/Oficinas/index', 'refresh');
                               exit;
                          }
          $data['web_js']  = array(                     
          ...
          $data['datos'] = $this->egeneral_model->mostrar('oficinas',$data_id);
          $data['tpl'] = 'backend/oficinas/editar';       
          $this->load->view('backend/layout/layout', $data);
      }
